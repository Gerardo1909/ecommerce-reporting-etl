"""
Pruebas unitarias para el validador de esquemas.
"""
...
class TestSchemaValidatorRequiredColumns:
    ...
    def test_validate_required_columns_should_fail_when_column_missing(
        ...
        check.is_in("falta", str(exc_info.value).lower())

    def test_validate_required_columns_should_fail_when_multiple_columns_missing(
        self, dataframe_missing_required_columns
    ):
...
class TestSchemaValidatorExtraColumns:
    ...
    def test_validate_no_extra_columns_should_fail_when_extra_column_exists(
    ...
    def test_validate_no_extra_columns_should_allow_subset_of_columns(
        self, sample_valid_dataframe
    ):
        """
...
class TestSchemaValidatorDataTypes:
    ...
    def test_validate_data_types_should_fail_when_type_mismatch(
    ...
    def test_validate_data_types_should_fail_when_column_not_exists(
        self, sample_valid_dataframe
    ):
...
class TestSchemaValidatorNumericRanges:
    ...
    def test_validate_numeric_range_should_fail_when_values_below_minimum(
        ...
        check.is_in("valores por debajo del mínimo", str(exc_info.value).lower())
        check.is_in("-5.0", str(exc_info.value))

    def test_validate_numeric_range_should_fail_when_values_above_maximum(
        self, dataframe_with_invalid_ranges
    ):
        ...
        check.is_in("valores por encima del máximo", str(exc_info.value).lower())
        check.is_in("150.0", str(exc_info.value))
    ...
    def test_validate_numeric_range_should_fail_when_nulls_not_allowed(
        self, dataframe_with_nulls
    ):
...
class TestSchemaValidatorNullValues:
    ...
    def test_validate_no_nulls_should_validate_specific_columns_only(
        ...
        error_msg = str(exc_info.value)
        check.is_in("id", error_msg)
        # No debe mencionar 'name' o 'price' porque no los estamos validando
        check.is_not_in("name", error_msg)

    def test_validate_no_nulls_should_pass_when_other_columns_have_nulls(self):
        """
        Debe pasar si solo validamos columnas sin nulos,
        incluso si otras columnas tienen nulos.
        """
        df = pd.DataFrame(
            {
                "id": [1, 2, 3],  # Sin nulos
                "name": [None, "B", None],  # Con nulos
    ...
    def test_validate_no_nulls_should_report_null_counts(self, dataframe_with_nulls):
        ...
        error_msg = str(exc_info.value)
        # Debe incluir información cuantitativa
        check.is_in("1", error_msg)  # Cantidad de nulos
...
class TestSchemaValidatorUniqueValues:
    ...
    Valida que las columnas contengan solo valores únicos (sin duplicados).
    """
    ...
    def test_validate_unique_values_should_fail_when_duplicates_exist(
    ...
    def test_validate_unique_values_should_report_duplicate_count(
        ...
        error_msg = str(exc_info.value)
        # Debe incluir información sobre duplicados
        check.is_in("1", error_msg)  # 1 valor duplicado (el 2)

    def test_validate_unique_values_should_fail_for_multiple_columns_with_duplicates(
        self, dataframe_with_duplicates
    ):
